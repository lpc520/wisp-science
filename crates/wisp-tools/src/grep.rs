...
impl Tool for GrepTool {
    ...
    async fn run(&self, args: &serde_json::Value, env: &dyn ToolEnv) -> ToolResult {
        ...
        let re = match Regex::new(&pat) { Ok(r) => r, Err(e) => return ToolResult::fail(format!("grep error: invalid regex: {e}")) };
        ...
        let out = if hits.is_empty() { "none".to_string() } else { hits.join("\n") };
        ToolResult::ok(out)
    }
}
