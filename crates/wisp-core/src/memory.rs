...
impl MemoryManager {
    ...
    fn today_path(&self) -> PathBuf {
        self.dir.join(format!("{}.md", chrono::Local::now().format("%Y-%m-%d")))
    }
    ...
    fn tokenize(text: &str) -> Vec<String> {
        text.split_whitespace().map(|s| s.to_ascii_lowercase()).filter(|s| !s.is_empty()).collect()
    }

    fn split_chunks(text: &str) -> Vec<String> {
        static RE: OnceLock<Regex> = OnceLock::new();
        let re = RE.get_or_init(|| Regex::new(r"\n\s*\n").unwrap());
        re.split(text).map(|c| c.trim().to_string()).filter(|c| !c.is_empty()).collect()
    }
    ...
    pub fn search(&self, query: &str, top_k: usize) -> String {
        ...
        if keywords.is_empty() { return "empty query".into(); }
        let pattern = format!("{}/*.md", self.dir.display());
        let mut scored: Vec<(i64, String, String)> = vec![];
        let mut paths: Vec<PathBuf> = glob(&pattern).ok().into_iter().flatten().filter_map(|r| r.ok()).collect();
        paths.sort_by(|a, b| b.cmp(a)); // newest filename first
        for path in paths {
            let Ok(text) = std::fs::read_to_string(&path) else { continue };
            let mtime = std::fs::metadata(&path).ok().and_then(|m| m.modified().ok()).and_then(|t| t.duration_since(std::time::UNIX_EPOCH).ok()).map(|d| d.as_secs() as i64).unwrap_or(0);
            for chunk in Self::split_chunks(&text) {
                let lower = chunk.to_ascii_lowercase();
                let mut score = 0;
                for kw in &keywords {
                    if lower.contains(kw) { score += lower.matches(kw).count() as i64 * 10; }
                }
                if score <= 0 { continue; }
        ...
        scored.into_iter().take(top_k).map(|(s, f, c)| format!("# {f} (score={s})\n{c}")).collect::<Vec<_>>().join("\n\n---\n\n")
    }
}
