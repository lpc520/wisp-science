...
fn rules() -> (CompactRule, CompactRule, CompactRule) {
    let tool = CompactRule { max_tokens: 800, keep_head: 200, keep_tail: 200, max_age: 21_600 };
    let reasoning = CompactRule { max_tokens: 500, keep_head: 125, keep_tail: 125, max_age: 7_200 };
    let assistant = CompactRule { max_tokens: 1500, keep_head: 350, keep_tail: 350, max_age: 10_800 };
...
impl ContextManager {
    ...
    pub fn disable_auto_compact(&mut self) { self.auto_compact_disabled = true; }
    ...
    pub fn clear_runtime_injections(&mut self) { self.runtime_injections.clear(); }

    pub fn append_assistant(&mut self, content: String, tool_calls: Vec<ToolCall>, reasoning: Option<String>) {
    ...
    pub fn append_tool(&mut self, tool_call_id: impl Into<String>, tool_name: impl Into<String>, content: wisp_llm::Content) {
    ...
    fn role_msgs(&self, role: Role, n: Option<usize>) -> Vec<&Message> {
        let slice = match n { Some(n) => self.get_latest(n), None => &self.messages[..] };
        slice.iter().filter(|m| m.role == role).collect()
    }

    fn tool_names_of(msgs: &[&Message]) -> Vec<String> {
        msgs.iter().filter(|m| m.role == Role::Tool).filter_map(|m| m.tool_name.clone()).collect()
    }

    #[allow(dead_code)]
    fn last_user_content(&self, msgs: Option<&[Message]>) -> String {
        let src = msgs.unwrap_or(&self.messages);
        src.iter().rev().find(|m| m.role == Role::User).map(|m| m.content.as_text()).unwrap_or_default()
    }

    fn under_threshold(&self) -> bool { self.total_tokens() < self.auto_compact_threshold }
    ...
    pub fn tool_context(&self, n: usize, cap: usize) -> String {
        ...
        for m in self.role_msgs(Role::Tool, Some(n)) {
            ...
            let compacted = if content.len() > cap { Self::compact_text(&content, 200, 200) } else { content.clone() };
    ...
    pub fn detect_loop(&self, threshold: usize) -> (bool, Option<String>) {
        ...
        if recent.len() < 10 { return (false, None); }
        ...
        for m in &recent {
            ...
            if Some(tool) == last_tool && (content.contains("fail") || content.contains("error")) {
                ...
                if Some(tool) != last_tool { fail_streak = 0; }
            }
            last_tool = Some(tool);
            if fail_streak >= threshold { return (true, Some(tool.to_string())); }
        }
        ...
        for m in tail {
            ...
            if c.contains("fail") || c.contains("error") {
                if let Some(n) = &m.tool_name { fail_set.insert(n.clone()); }
                fail_count += 1;
            }
        }
    ...
    pub fn detect_phase(&self) -> String {
        ...
        if is_looping { return "stuck".into(); }
        let all = Self::tool_names_of(&self.role_msgs(Role::Tool, None));
        if all.is_empty() { return "start".into(); }
        let recent = &all[all.len().saturating_sub(5)..];
        if recent.iter().any(|t| t == "edit" || t == "write") { return "executing".into(); }
        if recent.iter().all(|t| t == "read" || t == "grep" || t == "search") { return "exploring".into(); }
        if recent.iter().filter(|t| t == &"shell").count() >= 2 { return "verifying".into(); }
        "executing".into()
    }
    ...
    fn micro_compact(&mut self) {
        ...
        for m in &mut self.messages {
            ...
            if self.white_tool_list.iter().any(|w| w == name) { continue; }
            let content = m.content.as_text();
            if content.is_empty() || content.ends_with(' ') { continue; }
            if now - m.ts < tool_rule.max_age { continue; }
            if content.len() / 4 + 4 <= tool_rule.max_tokens { continue; }
            m.content = wisp_llm::Content::text(Self::compact_text(&content, tool_rule.keep_head, tool_rule.keep_tail));
        }
    }

    fn session_memory_compact(&mut self, retain_turns: usize) -> bool {
        let systems: Vec<Message> = self.messages.iter().filter(|m| m.role == Role::System).cloned().collect();
        let turns = self.split_turns();
        if turns.len() <= retain_turns { return false; }
        ...
        for turn in old {
            for mut m in turn.clone() {
                if m.role == Role::Tool {
                    ...
                    let content = m.content.as_text();
                    if !content.is_empty() && !content.ends_with(' ') && (content.len() / 4 + 4) > assistant_rule.max_tokens {
                        m.content = wisp_llm::Content::text(Self::compact_text(&content, assistant_rule.keep_head, assistant_rule.keep_tail));
                    }
                    if let Some(r) = m.reasoning.clone() {
                        if !r.is_empty() && !r.ends_with(' ') && (r.len() / 4 + 4) > reasoning_rule.max_tokens {
                            m.reasoning = Some(Self::compact_text(&r, reasoning_rule.keep_head, reasoning_rule.keep_tail));
                        }
                    }
                }
                compacted.push(m);
        ...
        for turn in recent { compacted.extend(turn.clone()); }
    ...
    fn compact_conversation(&mut self, retain_turns: usize) {
        ...
        while !trimmed_old.is_empty() {
            ...
            for turn in &trimmed_old { candidate.extend(turn.clone()); }
            for turn in recent { candidate.extend(turn.clone()); }
            if candidate.iter().map(Self::estimated_tokens).sum::<usize>() <= self.auto_compact_threshold {
                self.messages = candidate; return;
            }
        ...
        while trimmed_recent.len() > 1 {
            let mut candidate = systems.clone();
            for turn in &trimmed_recent { candidate.extend(turn.clone()); }
            if candidate.iter().map(Self::estimated_tokens).sum::<usize>() <= self.auto_compact_threshold {
                self.messages = candidate; return;
            }
            trimmed_recent.remove(0);
        }
        self.messages = systems.into_iter().chain(trimmed_recent.into_iter().flatten()).collect();
    }
...
8. Current Work";
        ...
        let comp = provider.complete(&messages, &[]).await.map_err(|e| format!("full compact err: {e}"))?;
        let summary = comp.content;
        if summary.trim().is_empty() { return Err("full compact err: llm respon null".into()); }
        let systems: Vec<Message> = self.messages.iter().filter(|m| m.role == Role::System).cloned().collect();
